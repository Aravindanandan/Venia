...
document.addEventListener('DOMContentLoaded', function() {
 ...
 function totallist(length){
  ...
  document.getElementById("allresultbtn").innerHTML = `See ${length} Results`
 }
...
function handleSortChange(selectedoption){
  ...
  function togglemenu(){
    const ele = document.getElementById("menu_items");
  ...
  function showfilter(){
    const ele = document.getElementById("filter_list");
    if(ele.style.display == "block"){
      ele.style.display= "none"
    }else{
      ele.style.display = "block";
